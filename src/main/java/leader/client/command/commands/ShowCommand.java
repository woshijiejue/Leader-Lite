    ...
    public void runCommand(ArrayList<String> args) {
        if (args.size() < 2) {
            ...
            if (module == null) {
                ...
                DebugUtil.sendFormatted(String.format("%s&o%s&r is not hidden in HUD&r", Leader.clientName, module.getName()));
            } else {
                module.setHidden(false);
                DebugUtil.sendFormatted(String.format("%s&o%s&r is no longer hidden in HUD&r", Leader.clientName, module.getName()));
            }
        } else {
            for (Module module : Leader.moduleManager.modules.values()) {
                module.setHidden(false);
            }
            DebugUtil.sendFormatted(String.format("%sAll modules are no longer hidden in HUD&r", Leader.clientName));
        }
    }
}
