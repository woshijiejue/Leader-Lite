package leader.client.component.impl.network.blink;
...
public class BlinkComponent implements InstanceAccess {
    public BlinkType blinkModule = BlinkType.NONE;
    @Getter
    ...
    public boolean offerPacket(Packet<?> packet) {
        if (this.blinkModule == BlinkType.NONE || packet instanceof C00PacketKeepAlive || packet instanceof C01PacketChatMessage) {
            return false;
    ...
    public boolean setBlinkState(boolean state, BlinkType module) {
        if (module == BlinkType.NONE) {
            return false;
        }
        if (state) {
            ...
            if (settings != null && settings.slowRelease.getValue() && settings.slowReleaseTime.is("Start Blink")) {
            ...
            BlinkSettings settings = getBlinkSettings();
            if (settings != null && settings.slowRelease.getValue() && settings.slowReleaseTime.is("Stop Blink")) {
                this.blinking = false;
                this.slowReleasing = true;
                this.slowReleaseTicks = 0;
                return true;
            ...
            if (Minecraft.getMinecraft().getNetHandler() != null && this.blinkedPackets.isEmpty()) {
                this.blinkModule = BlinkType.NONE;
                return true;
            }
    ...
    public BlinkType getBlinkingModule() {
    ...
    private void processSlowRelease() {
        ...
        slowReleaseTicks = 0;
        int maxTotal = settings.maxPacketsPerTick.getValue().intValue();
        int maxC03 = settings.maxC03PacketsPerTick.getValue().intValue();
        int released = 0;
        int c03Released = 0;
